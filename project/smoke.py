...
def test_input_shape():
    ...
    for count in range(N):
        ...
        x = torch.randn(B, C, H + h, W + w).clamp(0.0, 1.0)
        # print("x: ", x.size())
...
def run_bench_mark():
    ...
    with torch.profiler.profile(
        ...
        for ii in range(N):
            image = torch.randn(B, C, H, W).clamp(0.0, 1.0)
            with torch.no_grad():
...
def export_netdis_onnx_model():
    import onnx
    import onnxruntime
    from onnxsim import simplify
    import onnxoptimizer

    print("Export netdis onnx model ...")
...
def export_vitmat_onnx_model():
    import onnx
    import onnxruntime
    from onnxsim import simplify
    import onnxoptimizer

    print("Export vitmat onnx model ...")

    # 1. Run torch model
    model, device = image_matte.get_vitmat_trace_model() # get_vitmat_model()
    model = model.to(device)

    B, C, H, W = 1, 4, 256, 256 # model.MAX_H, model.MAX_W
    dummy_input = torch.randn(B, C, H, W).to(device).clamp(0.0, 1.0)
    with torch.no_grad():
        dummy_output = model(dummy_input)
    torch_outputs = [dummy_output.cpu()]
    ...
    onnx_filename = "output/image_vitmat.onnx"
